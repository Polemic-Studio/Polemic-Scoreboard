...
local SCOREBOARD_DISABLED_CONTROLS	= { "next_weapon",	-- Controls that are disabled when the scoreboard is showing
										...
										"radio_next",
										"radio_previous" }
local SCOREBOARD_TOGGLE_TIME		= 250				-- Time in miliseconds to make the scoreboard (dis)appear
...
drawScoreboard = function ()
	...
	local serverName = getElementData ( g_scoreboardDummy, "serverNam" ) or " Long Life - 0.2"
	...
	local strPlayerCount = "Gracze: " .. tostring(#g_players) .. "/" .. tostring(maxPlayers) .. " (" .. math.floor(usagePercent + 0.5) .. "%)"
	...
	-- Draw the headers
	drawRowBounded ( "ID", "Nazwa Gracza", "Godziny", "Ping",
	...
	for k=playersToSkip + 1, #g_players do
		...
		if not (hasPerk and tonumber(perkValue) == 1) or exports.global:isStaffOnDuty(localPlayer) then
